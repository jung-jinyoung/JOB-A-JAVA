...
> **5주차는 신규 개념(IoC/DI/AOP/동시성)과 스프링 부트 셋업, 이식 과제가 한 번에 몰려 있어 다른 주보다 부담이 큽니다.** 시간이 부족하면 우선순위는 `이식 과제 > 동시성 재현 과제 > 핵심 질문`순으로 두고, 동시성 재현 과제를 다 못 끝냈다면 `notes.md`에 어디까지 했는지와 막힌 지점을 남긴 채로 제출하세요. 미완성보다 "어디서 왜 막혔는지"가 리뷰에 더 도움이 됩니다.
